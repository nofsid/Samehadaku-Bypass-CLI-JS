...
const gdGetLink = async (url) => {
  ...
  const data = await _axios
    ...
    .then((data) => {
      return { success: true, json: JSON.parse(data.data.slice(4)) };
    })
    .catch(() => {
...
exports.GDriveDL = async (url, cb = () => {}) => {
  ...
  console.log("✅  " + _colors.green("Done"));
  if (!data.success)
    return console.log("❎  " + _colors.red("File not Found!"));
  if (data.json.downloadUrl === undefined)
    return console.log("❎  " + _colors.red("Link Download LIMIT"));
  console.log("⏳  " + _colors.yellow("Fetch Link Download..."));
  console.log("✅  " + _colors.green("Done"));
  console.log(
    "🎁  " +
      _colors.yellow("Start Download From URL : " + data.json.downloadUrl)
  );
