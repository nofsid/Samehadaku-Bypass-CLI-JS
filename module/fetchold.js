...
const getListSearch = (url, save = false) => {
  ...
  request(url, (err, res, body) => {
    ...
    if (!list.length) {
      console.log(
        `anime with query ${decodeURIComponent(
          querySearch("s", url)
        )} has not found`
      );
    } else {
      list.each((i, el) => {
        arrQuest.push(
          ...
          new inquirer.Separator(
            chalk.gray(`└> ${$(el).find(".date.meta-item").text().trim()}`)
          )
        );
        if (list.length - 1 === i && next.length)
          arrQuest.push({
        ...
        if (list.length - 1 === i && prev.length)
          arrQuest.push({
      ...
      .then((a) => {
        ...
        if (save) {
          ...
          $("li").each((i, el) => {
            let reso = $(el).find("strong").text().trim();
            arrQuest.push(
              new inquirer.Separator(chalk.green(`████ ${reso} ████`))
            );
            $(el)
              .find("a")
              .each((i, el) => {
...
const Pancal = (url, save = false, config = {}) => {
  ...
  request(url, (err, res, body) => {
    ...
    if (urlnya.length) {
      ...
      urlnya = Buffer.from(querySearch("r", urlnya), "base64").toString(
        "ascii"
      );
      next =
        [...shorturl, ...safeurl, ...safeurl2].indexOf(
          urlz
            .parse(urlnya)
      ...
      if (
        ...
        urlnya = decodeURIComponent(
          Buffer.from(urlnya, "base64")
            .toString("ascii")
            .substr(2)
            .split("")
            .reverse()
        ...
        urlnya = decodeURIComponent(querySearch("url", url));
        urlnya = decodeURIComponent(
          Buffer.from(urlnya, "base64").toString("ascii")
        );
        next =
          [...shorturl, ...safeurl, ...safeurl2].indexOf(
            urlz
    ...
    if (next) {
      ...
      if (save) {
        ...
        fs.writeFileSync(
          "./[" + config.type + "]" + config.name + ".txt",
          JSON.stringify(collect, null, 4)
        );
      } else {
    ...
    if (page < total)
      arrQuest.push({
        ...
        value: {
          page: true,
          url:
            "https://www.samehadaku.tv/wp-json/wp/v2/posts?per_page=14&page=" +
            (page + 1),
        },
      });
    if (page > 1)
